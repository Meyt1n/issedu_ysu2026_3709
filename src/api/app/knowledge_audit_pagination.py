"""Signed cursors for actor-scoped knowledge retrieval audit pages."""
...
_CURSOR_VERSION = 1
_CURSOR_RESOURCE = "knowledge-query-audits"


@dataclass(frozen=True)
class KnowledgeAuditCursor:
    actor_id: str
...
def encode_knowledge_audit_cursor(
    *,
    actor_id: str,
    household_id: str | None,
    member_id: str | None,
    ...
    """Encode the actor, filters and stable sort key, never audit contents."""
...
def decode_knowledge_audit_cursor(
    cursor: str, *, secret: str
) -> KnowledgeAuditCursor:
    """Validate a signed cursor and return its normalized scope and sort key."""
    try:
        ...
        if (
            ...
            or not actor_id
            or (household_id is not None and not isinstance(household_id, str))
        ...
        raise ValueError("KNOWLEDGE_AUDIT_CURSOR_INVALID") from None
