...
<template>
  <section class="page-hero member-portal-hero">
    ...
    <p class="hero-sub">这里显示家庭管理员已经确认的服药安排。需要调整时，请告诉家庭管理员。</p>
  </section>
  ...
  <section class="card">
    ...
    <div v-else-if="!workbench?.plans.length" class="empty-state member-empty"><AppIcon name="plan" :size="38" /><strong>暂时没有服药提醒</strong><p>家庭管理员创建安排后，会显示在这里。</p></div>
    <ul v-else class="list-plain member-plan-list">
      <li v-for="plan in workbench.plans" :key="plan.plan_event_id" class="member-plan-row">
        ...
        <span class="pill" :class="plan.status === 'ESCALATED' ? 'rose' : plan.status === 'REMINDER' ? 'gold' : 'pine'">{{ plan.status === 'ESCALATED' ? '需要家人关注' : plan.status === 'REMINDER' ? '提醒中' : '计划内' }}</span>
