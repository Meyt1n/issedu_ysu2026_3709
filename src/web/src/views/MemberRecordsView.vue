...
<template>
  ...
  <section class="card">
    ...
    <div v-else-if="events.length === 0" class="empty-state member-empty"><AppIcon name="compass" :size="38" /><strong>还没有确认过的记录</strong><p>药品照片经过家人确认后，会显示在这里。</p></div>
    <ul v-else class="list-plain member-record-list">
      <li v-for="event in events" :key="event.id" class="member-record-row">
        ...
        <div><strong>{{ eventTypeLabel(event.event_type, 'member') }}</strong><p>{{ summarizeEventPayload(event) || '家庭健康记录' }}</p></div>
