...
| MOB171-NET-10 Android 屏幕朗读 | 通过（设备持有人确认） | 荣耀 AAP-AN00 的“屏幕朗读”服务已开启；最新 Debug APK 在加载、连接超时、正在重试、成功加载四次状态切换时分别更新常驻 `role=status`、`aria-live=polite`、`aria-atomic=true` 播报区，失败卡同时使用 `role=alert`、`aria-live=assertive`。设备持有人确认屏幕朗读功能通过。 |
...
维护者仍需在最终候选上完成系统级断网→恢复、PWA 特大字号以及完整附件哈希回填；在这些事实源完成前，MOB-171 保持“待验收”，不能作为移动端发布通过证明。
