...
- 2026-08-31 扫描页预览与本机增强：虚线框内预览改为等比例铺满（`object-fit: contain`，不裁切、不拉伸）。点击质量检查时，若图片小于 640×480，先在浏览器内放大再送检；若结果含模糊/过暗/过亮/反光/尺寸不足等可修复原因，再做一次轻度对比度、曝光和锐化后复查。质量凭证仍绑定最终送检字节的 SHA-256；服务端质量模块仍不改写原文件。无法解码、主体裁切等硬失败仍只提示重拍。本增量不宣称已校准真实场景阈值，也不自动写入健康记录。
