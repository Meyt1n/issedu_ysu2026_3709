...
- 阶段：P0-W3
- 风险等级：R2
- 当前状态：进行中（后端质量门控与 #110 前端拍照/上传/重拍闭环已合并；#192 补充合成药盒取景示例并待合并；正式固定集校准仍未完成）
- 负责人：Meyt1n
- 复核人：维护者合并即完成人工复核
- 开始日期：2026-08-10
- 预计交付：2026-08-11
...
回滚时移除质量检查路由，或将配置回退到上一版已验证配置；视觉任务保持明确的预处理不可用状态；不得绕过质量门控自动写入健康事实。
...
- 2026-08-14 本地联调增量：质量门控切换为 `opencv-quality-demo-v2-lenient-exposure`。白色药盒在仍有黑色文字/边缘细节时不再仅因高亮像素或反光代理指标被拒绝；纯白、裁切或无可读细节的图像仍返回 `RETAKE`。使用 `xa_code` 环境中的 PaddleOCR worker 完成合成药盒端到端任务，首次模型加载约 15.8 秒；该结果仅证明本地链路可运行，不替代 HCT-201 固定集校准。
- 2026-08-14 本地 Demo 操作增量：`.env` 将 `VISION_QUALITY_ENFORCE_RETAKE` 设为 `false`，质量结果仍返回全部指标和诊断，但 API 不再因 `RETAKE` 阻止可解码图片进入 OCR；默认配置仍为严格模式，正式部署不得关闭该开关。
- 2026-08-18 前端取景增量（#192）：扫描页空状态增加合成药盒示例图和“正对药盒、完整入框、不要出现其他杂物”提示；示例图仅用于指导拍摄，不改变质量门控、识别或人工确认边界。
