...
仓库保留本地 OpenCV YuNet + SFace ONNX 的历史人脸凭证、校准和受控实验能力，**权重不进 Git**。HCT-498 起，成员前台和管理后台都只使用正式账号密码登录，欢迎页不再暴露人脸登录。需要维护历史凭证或执行受控实验时再准备模型：
...
- 两个登录页保持明显的入口品牌差异（HCT-455），但只保留同一种正式账号密码表单（HCT-498）：成员前台主按钮为「登录成员前台」，管理后台主按钮为「登录管理后台」。页脚互跳链接会自动带上 `?portal=member|admin`，即使目标端口没有注入入口模式也按正确品牌显示。
