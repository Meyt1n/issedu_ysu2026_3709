...
2. **药盒识别必须多渠道核对。** YOLO 负责定位，OCR 读取文字，条码/二维码提供码值，包装特征和本地主数据共同校验；冲突、未知或低质量结果进入人工复核，错误允许人工修正且保留原预测。
...
| 视觉模型 | YOLO11n 稳定基线与 YOLO26n 前沿对照 | YOLO11n 作为 P0 主线，YOLO26n 只做同数据同硬件对照，过门禁才替换 |
...
- YOLO：药盒、药瓶、药板、药名区、规格区、批号/有效期区、条码区；
- OCR：药名、规格、厂家、批号、生产日期、有效期；
- 条码/二维码：码值、解码质量和来源；
...
| 视觉 | YOLO11n 主线，YOLO26n 对照 | 模型版本、哈希、评估和回滚齐全 |
...
| 3 | 视觉 V1 | 授权数据、OpenCV、YOLO11n、图片/视频预处理 |
| 4 | 多证据复核 | OCR、条码、四态融合、人工修正、困难样本门禁 |
...
- YOLO/OCR 数据卡、模型卡、四态识别和 V1/V2 报告；
