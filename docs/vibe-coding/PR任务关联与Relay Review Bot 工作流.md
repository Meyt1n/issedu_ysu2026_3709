...
Bot 至少检查：任务和 Story 是否真的完成；验收证据是否可定位；测试、迁移、依赖、API/OpenAPI、文档和回滚是否同步；健康数据和密钥是否泄露；授权、撤权、过期、字段级可见范围和审计；健康事件是否确认、追加写并保护一致性；AI 是否越过证据边界进行诊断、处方、停药、换药或剂量判断。

阻断规则：任务结论不是“完成”，或存在 P0/P1 的必须修改项/风险，Job 失败；只有 P2 质量建议时 Job 可通过，但仍会在评论中列出。模型调用失败、返回非法 JSON、未配置接口或发现疑似密钥时 Job 失败，不能伪装成通过。
...
2. Relay Review Bot 发现未完成项或 P0/P1 风险：在原 PR 分支修复，补充测试/文档/验收证据后重新运行；
