...
- 未配置 API、接口超时、接口返回错误或模型返回非法 JSON：记录不可用告警，Relay Check 以降级成功结束，不阻塞其它 Required Checks；
...
4. 中转服务超时、不可达、未配置或返回非法结果时，Bot 会写入告警并以降级成功结束；维护者仍需自行完成最终检查，不得声称已经获得 AI Review；
