...
HCT-103 的 MySQL 不可变触发器要求服务端允许受限项目库账号创建触发器；仓库 Compose 固定 `log_bin_trust_function_creators=1`。已有事件时 `0004_hct103_event_recovery` 不允许破坏性 downgrade，必须停止 worker、保留事实和 outbox，并以前滚修复恢复。
