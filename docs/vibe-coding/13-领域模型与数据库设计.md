...
`id`、`family_id`、`member_id`、成员内 `sequence_no`、`event_type`、`occurred_at`、`recorded_at`、`actor_user_id`、`source_type`、`source_id`、`before_snapshot`、`after_snapshot`、`evidence_ids`、`recognition_model_version`、`rule_version`、`idempotency_key`、`request_fingerprint`、`correlation_id`、`causation_id`、`supersedes_event_id`、`schema_version`。

事件不可 UPDATE/DELETE；业务更正追加补偿事件。P0 使用家庭内幂等键和成员内单调序号；相同幂等指纹返回原结果，冲突指纹拒绝。敏感快照应最小化并加密，避免复制整份报告。

`outbox_message` 每个事件唯一，保存状态、尝试次数、可用/锁定/派发时间和稳定错误码；消息 payload 只含事件、家庭、成员、序号、确认和 Schema 版本，不复制健康正文。`member_state_projection` 保存最后事件/序号、投影版本和规范 JSON SHA-256；`projection_checkpoint` 保存本地 D2 状态快照、最后序号/事件和哈希。worker 回收过期锁并按成员序号派发；重复投递不增加投影效果，缺失已确认前序时保持 `OUT_OF_ORDER`。
...
HCT-103 的 MySQL 不可变触发器要求服务端允许受限项目库账号创建触发器；仓库 Compose 固定 `log_bin_trust_function_creators=1`。已有事件时 `0005_hct103_event_recovery` 不允许破坏性 downgrade，必须停止 worker、保留事实和 outbox，并以前滚修复恢复。
