...
`recognition_task` 记录任务类型、状态、输入文件引用、版本、阈值和耗时；`recognition_evidence` 保存帧/裁剪、框、OCR、条码、候选和冲突；`human_review` 保存决定、修正字段、理由、操作者和时间。确认事务同时追加事件并通过 outbox 触发投影/规则更新，避免“已确认但未更新状态”。
