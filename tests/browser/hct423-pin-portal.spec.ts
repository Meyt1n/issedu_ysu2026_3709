...
test('PIN 登录后自动进入成员前台并显示成员姓名', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: /家庭成员身份/ }).fill(member.actor_id)
  ...
  await expect(page.getByText('将以 奶奶 的身份进入成员前台。')).toBeVisible()
  ...
  await expect(page.getByText('已进入家庭成员前台')).toBeVisible()
})
