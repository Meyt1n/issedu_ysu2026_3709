...
- **家庭管理员**：沿用完整移动端导航，可查看家庭成员、提醒、风险依据和管理设置。
- **家庭成员**：联机后按家庭服务器中的家庭主人身份自动识别，只保留“今日 / 拍药盒 / 求助 / 我的”四个主入口；今日页只显示当前成员自己的任务、简化风险提示和已确认记录。
- 成员端隐藏家人列表、全家风险审计、知识库、趋势/操作历史和家庭 PIN 等管理内容；这些入口不会改变服务端的成员、字段和动作授权边界。演示模式可在“我的 → 手机端身份”手动切换两种展示，便于验收。

- **今日（管理员端）**：时段氛围英雄卡（任务进度环 + 滚动统计）+ 今日照护任务（确认 / 延期 / 跳过，跳过必填原因，完成反馈 Toast + 触觉震动，全部完成有彩带庆祝）+ **季节健康资讯小卡片**（家庭服务器白名单结果或虚构本地季节日历，明确来源/时间，可带标题与来源进入助手草稿）+ 近 7 天完成趋势图 + 待关注风险 + 最近变化时间线；
- **今日（成员端）**：仅显示当前成员的今日任务、简化“需要留意”提示、已确认记录和健康资讯；不显示全家选择器、环境行动卡、操作历史、趋势、风险依据等管理内容。页面仍可进入**语音助手**（联机后连接家庭服务器/电脑后端）；
