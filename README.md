...
面向 8 周软件工程实训的本地优先家庭健康项目。家健镜不替家庭成员看病，也不把“聊天机器人”或“定时闹钟”包装成创新；它持续记录家庭健康事实的变化，以多证据视觉录入为入口，通过确定性规则发现冲突、遗漏和风险，再用本地 RAG 与受约束大模型给出带证据的通俗解释和待确认任务。

> **当前状态：新项目方案已建立，业务代码尚未实现。**
>
> 仓库当前交付物是经过统一重写的需求、架构、数据、AI、安全、测试和实施基线。文档中的目标、接口与目录是开发契约，不代表功能已经完成；实际状态以[需求追踪矩阵](docs/vibe-coding/12-需求追踪矩阵.md)及可复现证据为准。
...
| 多证据视觉录入 | 图片/短视频质量检查、药品与报告区域检测、OCR、条码、人工复核 | 不承诺识别任意药品，不做持续家庭监控 |
| 家庭健康关系图谱 | 成员—疾病—过敏—药品—成分—计划—照护者关系投影 | P0 不引入大规模医学本体或自动医学推理 |
| 风险与环境规则 | 过期、临期、低库存、重复成分、过敏、有限相互作用、天气行动卡 | 不提供完整临床决策支持 |
| 受约束计划优化 | 服药确认、延期、跳过、照护升级及安全时间窗内提醒建议 | AI 不得新增、停用、替换药物或改变剂量 |
| 本地证据助手与大屏 | SQL/图谱/RAG 工具调用、风险解释、事件摘要、业务与模型指标 | 无证据不作肯定性医学回答 |
...
前端和管理员端不得直连数据库、向量库或 Ollama。MySQL 是唯一业务事实源；图谱是可重建投影；规则给出确定结论；LLM 负责工具选择、引用和语言解释。
...
docs/demo/          受控演示知识与场景
src/                待建设的 Web、API、AI 模块边界说明
tests/              待建设的单元、集成、E2E、模型和安全测试
migrations/         待建设的 MySQL 版本化迁移
...
1. 阅读[文档导航](docs/vibe-coding/00-文档导航.md)和[需求规格](docs/vibe-coding/01-需求规格说明书.md)。
2. 从[需求追踪矩阵](docs/vibe-coding/12-需求追踪矩阵.md)领取尚未完成的需求。
3. 按[任务拆分与交付清单](docs/vibe-coding/15-任务拆分与交付清单.md)建立 Story，并遵守[贡献指南](CONTRIBUTING.md)。
4. 只有在代码、测试和复现证据合并后，才把状态从“未开始/进行中”改为“已验证”。

本地启动命令尚不存在，不能用伪命令冒充可运行系统。环境建立后以[本地部署与 Demo 操作指南](docs/本地部署与Demo操作指南.md)为唯一入口。
